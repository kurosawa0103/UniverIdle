...
| `ScavengeHubView` | `WorkView_scavenge` | `detailPanel` → 拾荒 `Detail`；**必拖** `maps` → 各地图 `StandardWorkCenterView`（**无**运行时扫；一键绑定可补） |
| `StandardWorkCenterView` | **拾荒地图节点**（如 `Content/村口`）；挖矿/魔物可挂工作根 | `workId`、`sceneId`、动作卡；**必拖** `runningBarRoot` + fill/文案（已删运行时造 SceneTag） |
| `ActionListWorkCenterView` | `WorkView_woodcutting` | `workId`、动作卡、**必拖** `runningBarRoot` + fill/文案、`detailPanel` → 砍树 `Detail`（**无** Find RunningBar）；一键绑定：若误挂 `ScavengeHubView` 会换成本组件并补 `actionCards` |
...
| `ActionCardView` | 动作卡 | 标题、元信息、`thumbArt`（动作图）、`unlockText`、**`button`（`ClickButton`）**、`MasteryIcon` / `MasteryLevel`；砍树用 `Card_二狗家.prefab`（须含上述字段，勿残留已删 `thumb`）；Thumb 底板由预制体静态 Image，脚本不绑 `thumb`；Center Wire 用 `ClickButton`，不 `GetComponent` |
| `LootToastView` | **`App` 根下「获得提示区」**（全游戏一份；**勿**挂 `WorkView_*` / Detail） | **必拖** `lineRoot` / `floatLayer` / `linePrefab` / `floaterPrefab`；`MainUI.lootToast` 只拖此节点；结算可推道具 / 金币 / 经验；经验右侧为本轮挂机累计，停机清零 |
...
- **获得提示**：全游戏一份，挂 `App`（勿进拾荒/砍树 Detail）；行·飘字预制体只绑在 `LootToastView`；切工作不隐藏；一键绑定会挪到 App 并删多余副本；行须绑 `row`；道具/金币/经验行共用 `RefreshGainLine`；经验右侧为**本轮挂机累计**，`OnActionStopped` → `ResetSessionXp`
