...
namespace Fungus
{
    ...
    public class MenuDialog : MonoBehaviour
    {
        ...
        [Header("点击音效")]
        [Tooltip("菜单项被点击时播放；可在 MenuDialog 预制体上拖入 SE_Click1 等")]
        [SerializeField] protected AudioClip menuClickSfxClip;
        [Tooltip("PlayOneShot 音量系数")]
        [SerializeField] [Range(0f, 1f)] protected float menuClickSfxVolume = 1f;
        ...
        protected virtual void Awake()
        {
            ...
            if (Application.isPlaying && menuClickSfxClip == null)
            {
                menuClickSfxClip = Resources.Load<AudioClip>("Audio/SE/SE_Click1");
                if (menuClickSfxClip == null)
                    menuClickSfxClip = Resources.Load<AudioClip>("SE_Click1");
            }
        }

        /// <summary>
        /// 菜单点击音：使用 PlayClipAtPoint，避免选项回调里将本物体 SetActive(false) 后截断同一物体上的 PlayOneShot。
        /// </summary>
        protected virtual void PlayMenuClickSfx()
        {
            if (menuClickSfxClip == null || menuClickSfxVolume <= 0f) return;

            Vector3 pos = Camera.main != null ? Camera.main.transform.position : Vector3.zero;
            AudioSource.PlayClipAtPoint(menuClickSfxClip, pos, menuClickSfxVolume);
        }
        ...
        private bool AddOption(string text, bool interactable, bool hideOption, UnityEngine.Events.UnityAction action)
        {
            ...
            {
                PlayMenuClickSfx();
                MenuSignals.DoMenuOptionSelected(text);
                action();
            };
